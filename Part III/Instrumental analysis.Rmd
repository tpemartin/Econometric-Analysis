...
pums %>% group_by(qob) %>%
  summarise(mean.lwage=mean(lwklywge),
            mean.educ=mean(educ)) %>%
  mutate(qob2=factor(qob)) %>%
  ggplot(aes(x=qob))+
  geom_bar(aes(y=mean.educ),stat="identity")
```
...
col2
lm(lwklywge ~ educ + factor(yob) , pums)
```
OLS結果比較
```{r, warning=FALSE}
library(stargazer)
...
col6 <- ivreg(lwklywge ~ factor(yob) + factor(pob) + educ,
	          ...
	          pums)
col6
stargazer(col1,col2,col6,type='text',keep='educ',
          column.labels = c('OLS','OLS.yob.pob','IV'))
```
